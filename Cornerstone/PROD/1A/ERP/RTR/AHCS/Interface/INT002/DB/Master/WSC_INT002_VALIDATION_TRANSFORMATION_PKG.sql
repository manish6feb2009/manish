create or replace PACKAGE          "WSC_AHCS_AP_VALIDATION_TRANSFORMATION_PKG" AS 
------------------------------------------------------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------
   ...
   PROCEDURE leg_coa_transformation_reprocessing(p_batch_id IN number) ;
     FUNCTION  get_ledger_name ( p_header_id IN number,        p_batch_id IN number	) RETURN VARCHAR2;    
