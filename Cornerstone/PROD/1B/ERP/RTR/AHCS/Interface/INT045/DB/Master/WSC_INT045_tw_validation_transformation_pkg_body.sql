create or replace PACKAGE BODY "WSC_AHCS_TW_VALIDATION_TRANSFORMATION_PKG" AS
    ...
    --                    wsc_ahcs_tw_txn_line_t line,
    --                    wsc_ahcs_int_status_t     status
    ----                    wsc_ahcs_tw_txn_header_t  header
    --                WHERE
    --                        status.batch_id = p_batch_id
    ...
    --			   and line.header_id = header.header_id
        ...
        /*
        cursor cur_inserting_ccid_table is
            select distinct LEG_COA, TARGET_COA from wsc_ahcs_tw_txn_line_t
    ...
    --1. Identity the COA map corresponding to the source/ target system for the current batch ID (file name)        
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 15, 'Transformation start', NULL,
                      sysdate);
        OPEN cur_coa_map_id;
        ...
        logging_insert('TW', p_batch_id, 15.1, 'Transformation start', lv_coa_mapid
                                                                       || lv_tgt_system
                                                                       || lv_src_system,
                      sysdate);
    ...
    --2. Check data in cache table to find the future state COA combination
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 16, 'Check data in cache table to find ', NULL,
                      sysdate);
        BEGIN
    ...
    --				    and */ status.batch_id = line.batch_id
        ...
        EXCEPTION
            WHEN OTHERS THEN
                logging_insert('TW', p_batch_id, 22, 'Error in Check data in cache table to find', sqlerrm,
                              sysdate);
    ...
    --        update target_coa and attribute1 'Y' in ap_line table where distinct leg_coa and target_coa is null      
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 17, 'update target_coa and attribute1', NULL,
                      sysdate);
        BEGIN
         /*   open cur_leg_seg_value(lv_src_system, lv_tgt_system);
            ...
            forall i in 1..lv_leg_seg_value.count
                update wsc_ahcs_tw_txn_line_t set target_coa = lv_leg_seg_value(i).target_coa,  ATTRIBUTE1 = 'Y', 
                LAST_UPDATE_DATE = sysdate 
                where LEG_COA = lv_leg_seg_value(i).LEG_COA and batch_id = p_batch_id;
            end loop;
    */

            UPDATE wsc_ahcs_tw_txn_line_t tgt_coa
            SET
                target_coa = wsc_gl_coa_mapping_pkg.coa_mapping(lv_src_system, lv_tgt_system, tgt_coa.leg_bu, lpad(tgt_coa.leg_loc, 5,
                '0'), tgt_coa.leg_dept, tgt_coa.leg_acct, tgt_coa.leg_vendor, nvl(tgt_coa.leg_affiliate, '00000'), NULL, NULL, NULL, NULL),
                attribute1 = 'Y'
            WHERE
                    batch_id = p_batch_id
        ...
        EXCEPTION
            WHEN OTHERS THEN
                logging_insert('TW', p_batch_id, 19, 'Error in update target_coa and attribute1', sqlerrm,
                              sysdate);
                dbms_output.put_line('Error with Query:  ' || sqlerrm);
        END;
    ...
    --        insert new target_coa values derived from coa_mapping function into ccid_mapping table which values are for future use
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 16, 'insert new target_coa values', NULL,
                      sysdate);
        BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                logging_insert('TW', p_batch_id, 16, '.', sqlerrm,
                              sysdate);
        END;
    ...
    --      update ap_line table target segments,  where legal_entity must have their in target_coa
        ...
        -- 5. Update lines staging table with individual segment values split from the derived target segment concatenated values given by engine/         
              --cache tables.
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 18, 'update ap_line table target segments', NULL,
                      sysdate);
        BEGIN
            UPDATE wsc_ahcs_tw_txn_line_t
            SET
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Error with Query:  ' || sqlerrm);
                logging_insert('TW', p_batch_id, 27, 'Error in update ap_line table target segments', sqlerrm,
                              sysdate);
        END;
    ...
    --        if any target_coa is empty in ap_line table will mark it as transform_error in status table
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 19, 'if any target_coa is empty', NULL,
                      sysdate);
        BEGIN
            UPDATE (
                SELECT
            ...
            COMMIT;
            logging_insert('TW', p_batch_id, 19.1, 'updated attribute2 with TRANSFORM_FAILED status', NULL,
                          sysdate);
             ...
             where a.batch_id = p_batch_id
              ...
              (select 1 from WSC_AHCS_INT_STATUS_T b 
                where a.batch_id = b.batch_id 
                  ...
                  logging_insert('TW',p_batch_id,19.2,'updated attribute2 with TRANSFORM_FAILED status',null,sysdate);
        ...
        EXCEPTION
            WHEN OTHERS THEN
                logging_insert('TW', p_batch_id, 29, 'Error in if any target_coa is empty', sqlerrm,
                              sysdate);
                dbms_output.put_line('Error with Query:  ' || sqlerrm);
        END;
    ...
    --      update ledger_name in ap_header table where transform_success		
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 20, 'update ledger_name', NULL,
                      sysdate);
        BEGIN
    --         /*   open cur_get_ledger;
    --            loop
...
--    
--            MERGE INTO wsc_ahcs_tw_txn_header_t hdr
--            USING (
--                      WITH main_data AS (
...
--            SET hdr.ledger_name = e.ledger_name, hdr.LEG_BUS_UNIT = e.business_unit, hdr.TRANS_REF_NBR = e.business_unit;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                logging_insert('TW', p_batch_id, 22, 'Error in update ledger_name', sqlerrm,
                              sysdate);
                dbms_output.put_line('Error with Query:  ' || sqlerrm);
        END;
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 21, 'Update status tables after validation', NULL,
                      sysdate);
        /*begin
        ...
        exception
            when others then
                logging_insert('TW',p_batch_id,23,'Update status tables after validation',SQLERRM,sysdate);
                dbms_output.put_line('Error with Query:  ' || SQLERRM);
        end; */
        ...
        -------------------------------------------------------------------------------------------------------------------------------------------
        logging_insert('TW', p_batch_id, 22, 'Update status tables to have status', NULL,
                      sysdate);
        BEGIN
            UPDATE (
                SELECT
        ...
        logging_insert('TW', p_batch_id, 23, 'end transformation', NULL,
                      sysdate);
        logging_insert('TW', p_batch_id, 24, 'Dashboard Start', NULL,
                      sysdate);
        wsc_ahcs_recon_records_pkg.refresh_validation_oic(retcode, err_msg);
        logging_insert('TW', p_batch_id, 25, 'Dashboard End', NULL,
                      sysdate);
        logging_insert('TW', p_batch_id, 26, 'end transformation', NULL,
                      sysdate);
        DELETE FROM wsc_ahcs_tw_txn_tmp_t
        WHERE
            batch_id = p_batch_id;

        logging_insert('TW', p_batch_id, 27, 'deleted data from WSC_AHCS_TW_TXN_TMP_T for this batch_id', NULL,
                      sysdate);
    EXCEPTION
        WHEN OTHERS THEN
...
--                        AND substr(line.target_coa, 1, instr(line.target_coa, '.', 1, 1) - 1) IS NULL
    ...
    */
            ...
            UPDATE wsc_ahcs_tw_txn_line_t tgt_coa
            SET
                target_coa = replace(wsc_gl_coa_mapping_pkg.coa_mapping(lv_src_system, lv_tgt_system, tgt_coa.leg_bu, lpad(tgt_coa.leg_loc,
                5, '0'), tgt_coa.leg_dept, tgt_coa.leg_acct, tgt_coa.leg_vendor, nvl(tgt_coa.leg_affiliate, '00000'), NULL, NULL, NULL,
                NULL), ' ', ''),
                attribute1 = 'Y'
            WHERE
                    batch_id = p_batch_id
...
-- TODO: Implementation required for PROCEDURE WSC_AHCS_TW_VALIDATION_TRANSFORMATION_PKG.data_validation
        ...
        retcode               NUMBER;
        total_sum             NUMBER;
        total_records_line    NUMBER;
        total_records_control NUMBER;
...
/************* @@@@@ CHANGE THE TABLE TYPE & VARIABLE NAMES FOR THE HEADER & LINE TABLE TYPES  @@@@@@@@@@@ ***********************/
        ...
        lv_line_col_value     wsc_line_col_value_type := wsc_line_col_value_type('BUSINESS_UNIT', 'AXE_TRANS_DT', 'LEG_ACCT', 'LEG_DEPT',

        'LEG_LOC',
                                                                            'MONETARY_AMOUNT', 'FOREIGN_AMOUNT', 'CURRENCY_CD', 'FOREIGN_CURRENCY');

/************ @@@@@ INITIALIZE HEADER AND LINE FLAG TABLE TYPE VARIABLES TO MATCH THE COUNT OF HEADER AND LINE @@@@@ ************/
/************ @@@@@ CHANGE THE TABLE TYPE & VARIABLE NAMES FOR THE HEADER & LINE TABLE TYPES  @@@@@@@@@@@ ***********************/

--        TYPE wsc_ahcs_tw_header_txn_t_type IS
--        TABLE OF INTEGER;
--        lv_error_tw_header wsc_ahcs_tw_header_txn_t_type := wsc_ahcs_tw_header_txn_t_type('1', '1', '1');

        TYPE wsc_ahcs_tw_line_txn_t_type IS
            TABLE OF INTEGER;
        lv_error_tw_line      wsc_ahcs_tw_line_txn_t_type := wsc_ahcs_tw_line_txn_t_type('1', '1', '1', '1', '1',
                                                                                   '1', '1', '1', '1'); 

-------------------------------------------------------------------------------------------------------------------------------------------------------
-- Following cursor will fetch ALL COLUMN NAMES from LINE STAGING TABLE THAT MUST BE VALIDATED for data types
-------------------------------------------------------------------------------------------------------------------------------------------------------
        CURSOR cur_wsc_tw_line (
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    BEGIN
        logging_insert('TW', p_batch_id, 11, 'Start of validation', NULL,
                      sysdate);
        BEGIN
            SELECT
                attribute3
        ...
        logging_insert('TW', p_batch_id, 12, 'validation Line Count not Equal to Control Table Count', NULL,
                      sysdate);
        BEGIN   -- validation Line Count not Equal to Control Table Count
            SELECT
                total_records
            INTO total_records_control
            FROM
            ...
            SELECT
                COUNT(1)
            INTO total_records_line
            FROM
                wsc_ahcs_tw_txn_line_t
            WHERE
                batch_id = p_batch_id;

            IF total_records_control <> total_records_line THEN
                UPDATE wsc_ahcs_int_status_t
                SET
                    status = 'VALIDATION_FAILED',
                    error_msg = 'Validation Failed Line Count not Equal to Control Table Count',
        ...
        logging_insert('TW', p_batch_id, 13, 'Validate Mandatory Header and Line Columns Start', NULL,
                      sysdate);
        FOR header_id_f IN cur_header_id(p_batch_id) LOOP
--		        lv_header_err_flag := 'false';
--                lv_header_err_msg := NULL;
--
--                lv_error_tw_header := wsc_ahcs_tw_header_txn_t_type('1', '1', '1');
--                lv_error_tw_header(1) := is_date_null(header_id_f.transaction_date);
--                lv_error_tw_header(2) := is_varchar2_null(header_id_f.transaction_number);
--                lv_error_tw_header(3) := is_varchar2_null(header_id_f.file_name);
--
--				 FOR i IN 1..3 LOOP
----                    dbms_output.put_line(lv_error_tw_header(i));
--                    IF lv_error_tw_header(i) = 0 THEN
...
--                        COMMIT;
--
--                        logging_insert('TW', p_batch_id, 204, 'mandatory field validation failed for header' || header_id_f.header_id,
--                                  lv_header_err_flag,
--                                  sysdate);
--                        CONTINUE;               
--                END IF;
...
--
--
--      logging_insert('TW', p_batch_id, 14, 'HEADERID VALIDATION', header_id_f.header_id, sysdate);

            FOR wsc_tw_line IN cur_wsc_tw_line(header_id_f.header_id) LOOP
                lv_line_err_flag := 'false';
                lv_line_err_msg := NULL;
                lv_error_tw_line := wsc_ahcs_tw_line_txn_t_type('1', '1', '1', '1', '1',
                                                               '1', '1', '1', '1');

                BEGIN
                    lv_error_tw_line(1) := is_number_null(wsc_tw_line.business_unit);
                    lv_error_tw_line(2) := is_date_null(wsc_tw_line.axe_trans_dt);
                    lv_error_tw_line(3) := is_varchar2_null(wsc_tw_line.leg_acct);
                    lv_error_tw_line(4) := is_varchar2_null(wsc_tw_line.leg_dept);
                    lv_error_tw_line(5) := is_varchar2_null(wsc_tw_line.leg_loc);
                    lv_error_tw_line(6) := is_varchar2_null(wsc_tw_line.monetary_amount);
                    lv_error_tw_line(7) := is_varchar2_null(wsc_tw_line.foreign_amount);
                    lv_error_tw_line(8) := is_varchar2_null(wsc_tw_line.currency_cd);
                    lv_error_tw_line(9) := is_varchar2_null(wsc_tw_line.foreign_currency);
                EXCEPTION
                    WHEN OTHERS THEN
                        logging_insert('TW', p_batch_id, 15, 'ERR', sqlerrm,
                                      sysdate);
                END;

                FOR j IN 1..9 LOOP
                    IF lv_error_tw_line(j) = 0 THEN
...
--          logging_insert (null,p_batch_id,11,'lv_error_tw_line',null,sysdate);
                IF lv_line_err_flag = 'true' THEN
                    ...
                    WHERE
                        ...
                        AND status = 'NEW'
                        AND header_id = header_id_f.header_id
                        AND line_id = wsc_tw_line.line_id;
...
--                          sysdate);
            ...
            logging_insert('TW', p_batch_id, 204, 'mandatory field validation Completed' || header_id_f.header_id, lv_header_err_flag,
                          sysdate);

        END LOOP;   ---- Mandatory column Check completed

        logging_insert('TW', p_batch_id, 14, 'Validation Failed Line Amount not Equal to Zero', NULL,
                      sysdate);
        BEGIN   ---- line validation start here
--            SELECT
--                SUM(MONETARY_AMOUNT)
--            INTO total_sum
--            FROM
...
--            WHERE
--                batch_id = p_batch_id;
            FOR cur_mon_tx IN cur_mon_amt(p_batch_id) LOOP
  ...
  --          IF total_sum <> 0 THEN
                ...
                SET
                    status = 'VALIDATION_FAILED',
                    error_msg = '301|Validation Failed Line Amount not Equal to Zero',
                    reextract_required = 'Y',
                    attribute1 = 'L',
                    attribute2 = 'VALIDATION_FAILED',
                    last_updated_date = sysdate
                WHERE
                        batch_id = p_batch_id
                    AND header_id = cur_mon_tx.header_id;
...
--            END IF;
        ...
        logging_insert('TW', p_batch_id, 15, 'VALIDATION_SUCCESS Start', NULL,
                      sysdate);
        UPDATE wsc_ahcs_int_status_t
        SET
        ...
        COMMIT;
        logging_insert('TW', p_batch_id, 16, 'status updated', NULL,
                      sysdate);
        UPDATE wsc_ahcs_int_status_t
        SET
        ...
        CLOSE cur_count_sucss;
        logging_insert('TW', p_batch_id, 18, 'count success', lv_count_sucss,
                      sysdate);
        IF lv_count_sucss > 0 THEN
            ...
            COMMIT;
            BEGIN
                wsc_ahcs_tw_validation_transformation_pkg.leg_coa_transformation(p_batch_id);
            END;
        ELSE
            UPDATE wsc_ahcs_int_control_t
            SET
...
--            lv_src_system;
--        CLOSE cur_coa_map_id;
--        logging_insert('TW', p_batch_id, 15.1, 'Transformation start',
--                      lv_coa_mapid
--                      || lv_tgt_system
--                      || lv_src_system,
--                      sysdate);
...
--/************* @@@@@ CHANGE THE TABLE TYPE & VARIABLE NAMES FOR THE HEADER & LINE TABLE TYPES  @@@@@@@@@@@ ***********************/
--
--        lv_line_col_value   wsc_line_col_value_type := wsc_line_col_value_type('BUSINESS_UNIT',
--                                                                            'AXE_TRANS_DT',
--                                                                            'LEG_ACCT',
--                                                                            'LEG_DEPT',
--                                                                            'LEG_LOC',
...
--        END;
--        
--        logging_insert('TW', p_batch_id, 13, 'Validate Mandatory Header and Line Columns Start', NULL,sysdate);
--        
--        FOR header_id_f IN cur_header_id(p_batch_id) LOOP
----		        lv_header_err_flag := 'false';
----                lv_header_err_msg := NULL;
...
----                          sysdate);
--
--            logging_insert('TW', p_batch_id, 204, 'mandatory field validation Completed' || header_id_f.header_id,
--                          lv_header_err_flag,
--                          sysdate);
--
--        END LOOP;   ---- Mandatory column Check completed
--
--        logging_insert('TW', p_batch_id, 14, 'Validation Failed Line Amount not Equal to Zero', NULL,sysdate);
--        
--        BEGIN   ---- line validation start here
--            SELECT
--                SUM(MONETARY_AMOUNT)
...
--        END;
--        
--        logging_insert('TW', p_batch_id, 15, 'VALIDATION_SUCCESS Start', NULL,sysdate);
--        
--
--        UPDATE wsc_ahcs_int_status_t
--        SET
