...
--    DECODE(P_COA_MAP_NAME,'WESCO to Cloud',LPAD(pp_src_sgt2, 6, '0'),pp_src_sgt2);
    ...
    case when P_COA_MAP_NAME in ('Anixter to Cloud')  then '.'|| p_src_sgt5 when p_src_sgt5 is not null then '.'|| p_src_sgt5 else null end ||
    case when p_src_sgt6 is not null then '.'|| case when P_COA_MAP_NAME = 'Anixter to Cloud' then nvl(p_src_sgt6,'00000') else p_src_sgt6 end else null end ||
    ...
    begin
        lv_error_msg := case when upper(P_COA_MAP_NAME) = upper('WESCO to Cloud') and (p_src_sgt1 is null and p_src_sgt8 is not null) then 
            ...
            when upper(P_COA_MAP_NAME) = upper('Anixter to Cloud') and (p_src_sgt1 is null and p_src_sgt2 is not null and p_src_sgt3 is not null and p_src_sgt4 is not null) then 
    ...
    --dbms_output.put_line(lv_source_system);
             lv_tgt_sgt  := wsc_gl_coa_mapping_pkg.coa_mapping(lv_source_system,lv_target_system,
                ...
                p_src_sgt4,p_src_sgt5,case when P_COA_MAP_NAME = 'Anixter to Cloud' then nvl(p_src_sgt6,'00000') else p_src_sgt6 end,
    ...
    if  substr(lv_tgt_sgt,1,instr(lv_tgt_sgt,'.',1,1)-1) is not null then 
            ...
            lv_tgt_sgt,sysdate, sysdate, 'Y','N',p_user_name,p_user_name,p_src_sgt1, p_src_sgt2,p_src_sgt3,
                p_src_sgt4,p_src_sgt5,decode(P_COA_MAP_NAME,'Anixter to Cloud',nvl(p_src_sgt6,'00000'),p_src_sgt6),
                p_src_sgt7,p_src_sgt8,null,null);
            commit;
        end if;
            return lv_tgt_sgt;
...
--        update 
        ...
        exception
				when others then
                update WSC_GL_USER_COA_MAPPING_H set status='Failed', last_update_date = sysdate 
                where HEADER_ID = p_batch_id;
                commit;
					dbms_output.put_line('Error with Query:  ' || SQLERRM);
