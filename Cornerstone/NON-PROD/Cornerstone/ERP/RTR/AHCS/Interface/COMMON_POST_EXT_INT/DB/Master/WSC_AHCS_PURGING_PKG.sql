...
--- ****INTER INTRA COMPANY PURGING Added as a part of OC-132--
 ...
 /*********EBS FA PURGING ********************/
    ...
    Begin   
       Delete from wsc_ebs_fa_txn_t ebtxn
            Where exists (select 1 from wsc_gen_int_status_t
                where status='IMPORT_SUCCESS'
                AND legacy_header_id=ebtxn.assetid
                AND last_updated_date< trunc(sysdate)-p_success_days); --p_success_days
		...
		EXCEPTION
			WHEN OTHERS THEN
				lv_err_flag:='Y';
				lv_errm:=substr(lv_errm ||',wsc_ebs_fa_txn_t',1,4000);      
        END;


    Begin   
       Delete from wsc_gen_int_status_t
                where status='IMPORT_SUCCESS'
                AND last_updated_date< trunc(sysdate)-p_success_days;       
		...
		EXCEPTION
			WHEN OTHERS THEN
				lv_err_flag:='Y';
				lv_errm:=substr(lv_errm ||',wsc_gen_int_status_t',1,4000);      
        END;


    Begin   
       Delete from wsc_gen_int_control_t
                where status='IMPORT_SUCCESS'
                AND last_updated_date< trunc(sysdate)-p_success_days;
		...
		EXCEPTION
			WHEN OTHERS THEN
				lv_err_flag:='Y';
				lv_errm:=substr(lv_errm ||',wsc_gen_int_control_t',1,4000);      
        END;
       ...
       Delete from wsc_ebs_fa_txn_t ebtxn
            Where exists (select 1 from wsc_gen_int_status_t
                where status<>'IMPORT_SUCCESS'
                AND legacy_header_id=ebtxn.assetid
                AND last_updated_date< trunc(sysdate)-p_error_days);
