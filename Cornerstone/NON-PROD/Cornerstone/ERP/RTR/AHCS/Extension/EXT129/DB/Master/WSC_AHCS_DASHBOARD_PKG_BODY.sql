...
SELECT Clob_Val 
    ...
    FROM (SELECT Xmlcast(Xmlagg(Xmlelement(e, 
                                           ...
                                           Chr(10))) AS CLOB) AS Clob_Val, 
                 COUNT(*) AS Number_Of_Rows 
            FROM (SELECT 'FILE_NAME, SOURCE_COA, FUTURE_COA,ERROR_MSG, SOURCE_TRANSACTION_NUMBER, LEGACY_AE_HEADER_ID, LEGACY_LINE_NUMBER,STATUS,CR_DR,CURRENCY,AMOUNT,ACCOUNTING_DATE, AHCS CREATE ACCOUNTING REQ_ID, REPROCESS_REQUIRED,REEXTRACT_REQUIRED,TXN_NUMBER' AS Col_Value 
            /*added TXN_NUMBER column for defect (CTPFS-30362)*/
                    FROM Dual 
                  UNION ALL 
                  SELECT FILE_NAME||','||Source_COA||','||Target_COA||','||ERROR_MSG||','||Source_Trx_Num||','||Legacy_header_id||','||Legacy_Line_Number||','||Status||','||CR_DR_Indicator||','||Currency||','||Value||','||ACCOUNTING_DATE||','||AHCS_CREATE_ACCOUNTING_REQ_ID||','||REPROCESS_REQUIRED||','||REEXTRACT_REQUIRED||','||TXN_NUMBER AS Col_Value 
            /*added TXN_NUMBER column for defect (CTPFS-30362)*/
