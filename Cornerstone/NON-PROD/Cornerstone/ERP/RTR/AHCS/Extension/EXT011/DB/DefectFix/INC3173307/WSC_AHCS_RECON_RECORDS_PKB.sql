...
BEGIN
    ...
    BEGIN
        --wsc_ahcs_recon_records_pkg.vlog ('BEGIN TRUNCATE WSC_AHCS_RECON_REP_TEMP_T','UPDATE_REC');
        EXECUTE IMMEDIATE 'Truncate table WSC_AHCS_RECON_REP_TEMP_T';
        --wsc_ahcs_recon_records_pkg.vlog ('TRUNCATE WSC_AHCS_RECON_REP_TEMP_T Completed','UPDATE_REC');
    EXCEPTION
        WHEN OTHERS
        THEN
            --NULL;
...
BEGIN
    ...
    THEN
        UPDATE SET
            wais.attribute4 =
                NVL2 (attribute4, attribute4 || ',', NULL) || temp.JOB_ID,
...
Exception
    ...
    ERRBUF := 'ERROR While Updating Records'; 
    RECTCODE :=2;
        wsc_ahcs_recon_records_pkg.vlog('Main WHEN OTHERS '|| SUBSTR(SQLERRM,500) || DBMS_UTILITY.format_error_backtrace
    ,'REFRESH_VALIDATION_OIC');
...
BEGIN
    ...
    EXCEPTION
    WHEN OTHERS THEN
        wsc_ahcs_recon_records_pkg.vlog('WHEN OTHERS '|| SUBSTR(SQLERRM,500) || DBMS_UTILITY.format_error_backtrace
    ,'VLOG');
    END;
